...
router.post('/submit', function(req, res, next) {
  ...
  //2.整理公共数据|库数据
  let {name,content,price,rest} = req.body;
  ...
  // let auth_icon = req.files.length ? uploadUrl + req.files[0].filename + pathLib.parse(req.files[0].originalname).ext : '';
    let auth_icon=req.files.length;
    if(auth_icon){
      auth_icon=[];
      for(var i=0;i<req.files.length;i++){
        auth_icon.push("../"+uploadUrl + req.files[i].filename + pathLib.parse(req.files[i].originalname).ext)
        fs.renameSync(
          req.files[i].path,
  ...
  mgdb({
    ...
    collection.insertOne({
      name,price,time,rest,auth_icon
    },(err,result)=>{
